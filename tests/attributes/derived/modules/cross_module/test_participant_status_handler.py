...
def base_data() -> Dict[str, Any]:
    """Base data for testing; need to set uds/rejoin relative
    to these based on whatever is being tested."""
    return {
...
class TestParticipantStatusHandler:
    ...
    def test_status_no_overrides_uds(self, base_data) -> None:
        """Tests the latest UDS visit does NOT override other statuses
        because it is before the other dates."""
        base_data.update({
    ...
    def test_status_no_overrides_rejoin(self, base_data) -> None:
        """Tests the latest rejoin does NOT override other statuses
        because it is before the other dates."""
        base_data.update({
    ...
    def test_initial_visit_only_handling(self, base_data) -> None:
        """Test initial visit only is handled correctly. Only ever set
        if it is the ONLY thing ever set."""
        prespart = {
            "prespart": {
                "value": 1,
                "date": "2020-01-10"
