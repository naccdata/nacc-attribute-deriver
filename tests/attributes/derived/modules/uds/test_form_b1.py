...
class TestUDSFormB1Attribute:
    ...
    def test_create_naccbmi(self, uds_table):
        ...
        # test form not submitted; on initial visit should be -4
        uds_table["file.info.forms.json"].update(
    ...
    def test_compute_average_use_minimum(self, uds_table):
        """Test _compute_average works as expected when the use_minimum flag is
        involved."""
        ...
        assert attr._compute_average("field1", "field2", 30, 140) == 888
        assert (
            attr._compute_average("field1", "field2", 30, 140, use_minimum=True) == 13
        )
        ...
        assert attr._compute_average("field1", "field2", 30, 140) == 888
        assert (
            attr._compute_average("field1", "field2", 30, 140, use_minimum=True) == 888
        )
