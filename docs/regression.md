...
This file is used to do the mapping instead. Any write-ins that ClaudeAI was unable to determine was later backfilled by comparing the newly computed vs legacy QAFs. There are also mismatches where ClaudeAI disagrees with the legacy system, but in pretty much all cases it's completely subjective based on how you interpret the text. For example, in one case, the write-in value was simply "Insulin", so the new system matched it to `d00262: INSULIN`. However, it appears the old system instead matched it to `d04369: INSULIN REGULAR`. Similarly for things that were clearly misspelled, the two could have interpreted completely different drugs depending on the misspelling. As such, for such conflicts, we just kept the ClaudeAI interpretation, as it would currently require a heavy lift to investigate every single one of these cases.
