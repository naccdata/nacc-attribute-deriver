...
def discover_collections():
    ...
    plugin example shows due to the way Pants selectively imports files (the
    files "don't exist" if it's not imported to begin with).

    So currently doing the hacky thing where we just go ahead and
    explicitly import everything under mqt/nacc, so we're not really
...
def generate_attribute_schema(
    ...
    def evaluate_grouping(
            grouping: List[AttributeCollection]) -> List[Dict[str, Any]]:
        schema = []
