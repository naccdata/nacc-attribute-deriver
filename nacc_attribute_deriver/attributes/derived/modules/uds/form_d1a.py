...
class UDSFormD1aAttribute(UDSFormDxAttribute):
    ...
    def _create_naccmcii(self) -> int:
        ...
        is why _prev_record logic isn't used here.
        ...
        of this variable. As such, the output now is unlikely to match results
        generated by the legacy system.
        We are assuming that going forward it has been "corrected".
        """
        ...
        # we can assume the "NORMCOG = 1 or IMPNOMCI = 1" at IVP = true cases.
        # this is the same as ivcstat == 1; technically ivcstat can = 0 as well,
        # but as mentioned in the comments below that's kind of the default
        # fall-back case that shouldn't actually be set in practice. either way
        # if we got here it has to be one of those.

        # tldr; all we care about is the current MCI status. so just return that :)
    ...
    def _create_ivcstat(self) -> Optional[int]:
        ...
        Helper variable for FVMCI and NACCMCII. Only defined at initial visit.
        ...
        0 - All other cases (not sure if possible with this reworking but head
            hurts thinking about it and we need some sort of default)
