...
def load_v1_drugs() -> Dict[str, str | None]:
    ...
    In UDS V1 all the drugs were written in. Uses drug_ids_v1.csv,
    which was manually generated by comparing the raw values with
    expected QAF values. The list itself is imperfect/mappings are
    debatable but really just designed to produce the same drugs
    list as the legacy system.
    """
