...
It is arguably overkill to write a _missingness_ function for
every variable, when many could call a common rule instead (e.g. see
MINTTOTS-dependent variables) but due to the sheer number of them
and slight nuances I decided to be extremely explicit here to make
sure we aren't accidentally missing or duplicating any.
"""
...
class UDSFormC1C2Missingness(UDSMissingness):
    ...
    def _handle_verbaltest_gate(self, verbal_value: int, set_value: int) -> Optional[int]:
