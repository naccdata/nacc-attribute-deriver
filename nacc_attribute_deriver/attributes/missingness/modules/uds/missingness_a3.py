...
Similarly if no new info is provided (assured blank when NWINF variables = 0), then we
also need to pull through the previous visit. If NWINFx is set, enforce ignoring the
current value even if they set something (shouldn't happen due to error checks but
done as a safeguard.)
...
class UDSFormA3V4Missingness(UDSMissingness):
    ...
    def __handle_a3_prev_visit_missingness(
        self, nwinf_field: str, field: str, attr_type: Type[T], prev_code: int
    ) -> T:
        ...
        *ETPR and *ETSEC variables are effectively int-like strings, so can use
        ...
        return self.handle_prev_visit(
            field,
            attr_type,
            prev_code=attr_type(prev_code),  # type: ignore
            default=default,
